...
-  let , const - has declar hoist but no initialization hoist
-  var has hoisiting, in both declaration and initialization
...
```js
const fruit = { name: 'apple', color: 'red' };
//假如名字name已经用过了， 那么可以用这种方式将名字改成fruitname
...
---
- arrow function, regular function (function expression, function declaration)
- arrow function, regular function 中的this binding 是不一样的
```
关于this

1. 如果一个对象里面的方法中用到了this,那么就只指的是对象本身
2. 如果一个对象里面的一个 rugular function声明里用到了this(不是method),那么就指的是window， global object, **主要还是看是谁调用的就是谁**
3. 如果一个对象里面的arrow funciton用到this， 那么这个this指的是包裹这个arrow fucntion的东西（这个东西不能是对象，对象不属于一个作用域）的所属对象。
(it s always the "this" of its outer function)
(就是看他的outer function的this代表哪个值)![[2023-1-6-2.png]]
```
![[2023-1-6-2.png]]
##### 关于call()和bind() 可以绑定this的问题
##### call(), apply(), bind()
- 一个函数.call(一个对象)  ----- 就是说一个对象借用了一个函数（就是说这个函数中的this指代的是这个对象）  
- call就是执行一次，但是bind就像下面例子一样，永久绑定
![[屏幕截图 2023-01-06 140830.png]]
- apply,第一个参数指定调用函数的this所指代的对象，第二个参数是一个数组用来存放这个函数本身需要的参数， 而call却没有提供函数参数的地方
![[2023-1-6-1.png]]
...
1. 如果一个对象里面的方法中用到了this,那么就只指的是对象本身
2. 如果一个对象里面的一个function声明里用到了this(不是method),那么就指的是window， global object
3. 如果一个对象里面的arrow funciton用到this， 那么这个this指的是包裹这个arrow fucntion的东西（这个东西不能是对象，对象不属于一个作用域）的所属对象。
```
